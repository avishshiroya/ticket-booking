...
var refreshToken ='Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NWUwNjdmZDc0MjFkZGVjODdkMzA1ZGEiLCJpYXQiOjE3MTAyMjQwMjYsImV4cCI6MTcxMDgyODgyNn0.RsXnwo4ZAo_zM4rwXb_6cFAoxIzSCpoLrNk2qrJtV10';
describe('Tasks API', () => {
    describe("POST /", () => {
    ...
    describe("PUT /update-detail", () => {
        it("UPDATE THE USER DETAILS", async () => {
            const response = await request.put('/user/update-detail')
            .set('content-type','application/json')
                    .set('auth', auth)
                    .send({
                    name: "avish"
    ...
    describe("GET /orders", () => {
        it("ORDERS OF USER", async () => {
            const response = await request.get('/user/orders')
                .set('auth', auth)
                // .send({
                //     auth:auth
                // })
    ...
    describe("GET /refresh",()=>{
        it("GET New Access TOken",async ()=>{
            const response = await request.get('/user/refresh')
            .set('auth', auth)
