...
import { getRedis, setRedis } from "../middleware/redis.js";
...
export const addTrainSeatController = async (req, res) => {
  ...
  try {
    ...
    const checkClass = await trainModel.findOne({
      _id: checkTrainSlot.trainId,
      classes: { $in: [seatClass] },
    });
    if (!checkClass) {
...
export const updateTrainSeatController = async (req, res) => {
  try {
    ...
    const checkSeat = await trainSeatModel
...
export const getTrainSeatBySlotIdController = async (req, res, next) => {
  try {
    ...
    const cache = await getRedis(`trainSeats:${req.params.id}`);
    // console.log(JSON.parse(cache));
    if (cache) {
      console.log("cacahe 1");
    ...
    // if (cache.has(`trainSeats/${req.params.id}`)) {
    //   return successResponse(res, {
    //     statusCode: 200,
    //     data: [cache.get(`trainSeats/${req.params.id}`)],
    //     message: "trainSeats",
    //     header: { "X-name": "avish shiroya", "X-newOne": "abc" },
    //   });
    // }
    ...
    if (!getSeats[0]) {
      ...
      //     data:null
      // })
      //  return errorResponse(res, { statusCode: 404, message: "Seats Not Found" });
      // throw new Error('seats not FOund')
      throw new Error("seats not Found OF Train");
    }
    // res.status(200).json({
    //     status:"success",
    //     message:"train seats",
    ...
    // cache.set(`trainSeats/${req.params.id}`, getSeats);
   await setRedis(`trainSeats:${req.params.id}`, getSeats);
    logger.info(
      `${req.method} ${req.originalUrl} ${req.headers["user-agent"]} GetTrain Seat By Slot`
    );
    successResponse(res, {
      statusCode: 200,
      data: [getSeats],
    ...
    //   // })
    //   logger.error(
    //     `${req.method} ${req.originalUrl} ${req.headers["user-agent"]} Internal Server Error`
    //   );
